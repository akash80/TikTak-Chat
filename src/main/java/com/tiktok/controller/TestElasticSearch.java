...
//    public static void main(String args[]){
  ...
  Node node     = nodeBuilder().node();
        ...
        client.prepareIndex("application", "messages", "1")
              .setSource(putJsonDocument("Akash80",
                                         "abhi80",
                                         "hello bro",
                                         "12")).execute().actionGet();
        client.prepareIndex("application", "messages", "2")
              .setSource(putJsonDocument("Akash80",
                                         "abhi80",
                                         "what's up",
                                         "14")).execute().actionGet();
                
        getDocument(client, "application", "messages", "1");

        searchDocument(client, "application", "messages", "1", "ElasticSearch");
        
        
        node.close();
    }
    
    public static Map<String, Object> putJsonDocument(String sender, String receiver, String message, 
                                                      String id){
        
        Map<String, Object> jsonDocument = new HashMap<String, Object>();
        
        jsonDocument.put("sender", sender);
        jsonDocument.put("receiver", receiver);
        jsonDocument.put("message", message);
